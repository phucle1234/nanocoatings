...
(function(){
  function make(el){
    ...
    function step(){
      ...
      idx = (idx + Math.max(0.01, spd)) % total;
      render();
    ...
    // drag
    let dragging=false, lastX=0, startIdx=0, wasPlayingBeforeDrag=playing;
    const down = (e)=>{
      dragging=true;
    ...
    range.addEventListener('input', ()=>{ spd=parseFloat(range.value); if (playing){ startLoop(); } });
  }
