...
class PostCategoryCrudController extends CrudController
{
    ...
    public function setup()
    {
        ...
        CRUD::setRoute(config('backpack.base.route_prefix') . '/post-category');
        CRUD::setEntityNameStrings('danh mục tin tức', 'Danh mục tin tức');
    }

    protected function setupListOperation()
    {
        $layoutSlugs = array_filter([
            config('homepage_layout.category_slug'),
            config('homepage_layout.category_slug_en'),
        ]);
        CRUD::addClause('whereDoesntHave', 'translations', function ($query) use ($layoutSlugs) {
            $query->whereIn('slug', $layoutSlugs);
        });
        ...
        CRUD::addClause('where', 'is_banner', false);

        // Nếu có tham số category_id khi truy cập từ nút "Xem tin tức", lọc danh sách bài viết theo danh mục đó
        // Không áp dụng lọc ở đây
    }
    ...
    protected function setupCreateOperation()
    {
        ...
        CRUD::field('icon')
            ->label('Icon')
            ->type('text')
            ->attributes(['placeholder' => 'fas fa-newspaper'])
            ->hint('Font Awesome icon class')->tab('Chung');
    ...
    public function store()
    {
        ...
        Alert::success('Danh mục tin tức đã được tạo thành công!')->flash();
    ...
    public function update()
    {
        ...
        Alert::success('Danh mục tin tức đã được cập nhật thành công!')->flash();
        return $this->crud->performSaveAction($item->getKey());
    }
    ...
    protected function addMultilangFieldsWithSections()
    {
        ...
        foreach ($languages as $lang => $langName) {
            ...
            CRUD::field('url_' . $lang)
                ...
                ->hint('Nhập URL liên kết cho danh mục (nếu có)')
                ->tab($langName);
    ...
    public function destroy($id)
    {
        ...
        if (app(HomepageLayoutService::class)->isLayoutCategory($category)) {
            \Prologue\Alerts\Facades\Alert::error('Không thể xóa danh mục sắp xếp trang chủ.')->flash();
