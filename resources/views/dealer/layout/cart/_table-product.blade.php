    ...
    <table class="table table-hover">
        ...
        <tbody>
            @forelse ($list as $product)
                <tr data-product-id="{{ $product->id }}" data-url="{{ route('dealer.cart.add-to-cart') }}">
                    <td class="ps-0">
                        <a href="{{ route('dealer.cart.product-detail', ['id' => $product->id]) }}">
                    ...
                    <td class="text-nowrap text-end">
                        @if ($product->price > 0)
                            <span
                                class="product-price fs-14 fw-600">{{ number_format($product->price ?? 0, 0, ',', '.') }}đ</span>
                        @else
                    ...
                    <td class="pe-0">
                        <div class="d-flex align-items-center gap-2 justify-content-end">
                            <a href="{{ route('dealer.cart.product-detail', ['id' => $product->id]) }}"><i
                                    class="bi bi-eye fs-22"></i></a>
                            @if ($product->price > 0)
                                <span role="button" class="btn-add-to-cart"><i
                                        class="bi bi-cart-plus fs-22"></i></span>
                            @endif
                        </div>
                    </td>
                </tr>
