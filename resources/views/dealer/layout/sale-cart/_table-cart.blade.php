        ...
        <tbody>
            @foreach ($cart?->items ?? [] as $item)
                <tr data-item-id="{{ $item->id }}">
                    <td>
                        <a href="{{ route('dealer.sale-cart.product-detail', ['id' => $item->product->id]) }}"
                            target="_blank">
                            <div
                    ...
                    <td class="text-center">
                        @if ($item->options)
                            <span class="fw-500">{{ $item->quantity }}</span>
                        @else
...
@if ($type === 'index')
    ...
    @if ($hasErrorItem > 0)
        <div class="text-danger bg-light fs-14 fw-500 text-center border border-danger rounded-1 p-2">Có
            {{ $hasErrorItem }} sản phẩm gặp vấn đề về tồn kho
            hoặc giá, vui lòng kiểm tra lại, và xóa chúng trước khi tiếp tục.</div>
    @endif
    <div class="order-btn-link d-flex align-items-center justify-content-between gap-2 mt-5">
        ...
        <div class="d-flex align-items-center gap-3">
            ...
            @else
                <a href="{{ route('dealer.sale-checkout') }}"
