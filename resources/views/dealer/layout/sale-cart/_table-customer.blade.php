        ...
        <tbody>
            @forelse ($customers as $customer)
                <tr>
                    <td class="py-4 fs-18">
                        <div>
                            <input class="form-check-input check-item" type="checkbox" name="customer_code"
                                value="{{ $customer->code }}" aria-label="...">
                        </div>
                    </td>
                    <td class="py-4 fs-14">
                        <a href="{{ route('dealer.customer-detail', $customer->id) }}"
                            class="fw-500">#{{ $customer->code }}</a>
                    </td>
                    <td class="py-4 fs-14">
                    ...
                    <td class="py-4 fs-14">
                        <span class="fw-500">88.323.363đ</span>
                    </td>
                    <td class="py-4 fs-14">
                    ...
                    <td class="text-end">
                        <div class="d-flex align-items-center justify-content-end gap-3">
                            <a href="{{ route('dealer.customer-detail', $customer->id) }}"><i
    ...
    <script>
        (function() {
            document.addEventListener('change', function(e) {
                ...
                const anyChecked = Array.from(document.querySelectorAll('.check-item')).some(cb => cb.checked);
                const form = document.querySelector('.new-customer-info');
                if (form) {
                    if (anyChecked) {
                        form.classList.add('d-none');
                        document.querySelector('#check-create-new').checked = false;
                    } else {
