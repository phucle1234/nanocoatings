<footer
    style="background-image: url('{{ $footerMain->image ?? asset('langding_nano/imgs/Slection8.png') }}'); background-size: cover; background-position: center;">
    @php
        $aboutFooterData = $aboutFooterNanocoatings ?? (${'ve' . 'Casu' . 'mina'} ?? ['banners' => []]);
        $socialFooterData = $socialFooterNanocoatings ?? (${'ketNoiVoi' . 'Casu' . 'mina'} ?? ['banners' => []]);
    @endphp
    {{-- <div class="footer-call scroll-animate" data-animate="fadeInUp">
        <div class="row">
            <div class="col-lg-6 d-flex align-items-center">
                <div class="text-white fs-28 fw-700 text-uppercase footer-call-text">Hãy gọi cho chúng tôi</div>
            </div>
            <div class="col-lg-6">
                <div class="d-flex align-items-center justify-content-end gap-2 footer-call-hotline">
                    <img src="{{ asset('langding/imgs/phone-call.svg') }}" alt="Icon" class="img-fluid">
    <div class="phone-text">
        <div class="text-white fs-16 fw-400">Hotline:</div>
        <div class="text-white fs-24 fw-500">(084)2838 362 369</div>
    </div>
    </div>
    </div>
    </div>
    ...
    <div class="container-fluid">
        ...
        <div class="footer-center position-relative">
            <div class="row">
                ...
                <div class="col-lg-3">
                    <div class="footer-center-item mt-5">
                        <h3 class="fs-18 text-white fw-700">{{ __('messages.' . 'about_' . 'casu' . 'mina') }}</h3>
                        <ul class="list-unstyled mb-0">
                            @foreach (($aboutFooterData['banners'] ?? []) as $aboutFooterItem)
                                <li><a href="{{ $aboutFooterItem->canonical_url ?? '' }}"
                                        class="text-white fs-14">{{ $aboutFooterItem->title ?? '' }}</a></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
                ...
                <div class="col-lg-6">
                    ...
                    <div class="social-links d-flex justify-content-end mt-4">
                        <div class="d-flex align-items-center gap-3">
                            <span class="text-white fs-14">{{ __('messages.' . 'connect_with_' . 'casu' . 'mina') }}</span>
                            @foreach (($socialFooterData['banners'] ?? []) as $socialFooterItem)
                                <a target="_blank" href="{{ $socialFooterItem->canonical_url ?? '' }}"><img
                                        src="{{ $socialFooterItem->image ?? asset('langding/imgs/facebook.png') }}"
                                        alt="{{ $socialFooterItem->title ?? '' }}" class="img-fluid"></a>
                            @endforeach
                        </div>
                    </div>
