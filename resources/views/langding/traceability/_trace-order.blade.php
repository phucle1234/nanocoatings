            ...
            <div class="col-sm-9">
                <div class="warranty-certification-info py-4 px-3 ps-sm-0 pe-sm-3">
                    <p class="fs-16 fw-600 mb-1 text-uppercase">
                        {{ __('messages.traceability_certification.customer_name_label') }}
                    </p>
                    <h2 class="font-hanzel fs-26 text-red">{{ $traceInfo?->customer_name ?? 'N/A' }}</h2>
                    <div class="d-flex justify-content-between mt-4">
                        <div class="warranty-certification-status">
                            ...
                            <p class="text-red fw-600">{{ $traceInfo?->order_no ?? 'N/A' }}</p>

                            <p class="fw-500 mb-1 fs-14">{{ __('messages.traceability_certification.phone_label') }}</p>
                            <p class="text-red fw-600">{{ $traceInfo?->phone ?? 'N/A' }}</p>

                            <p class="fw-500 mb-1 fs-14">Email</p>
                            <p class="text-red fw-600">{{ $traceInfo?->email ?? 'N/A' }}</p>
                    ...
                    <div class="d-flex align-items-center gap-2 mt-3">
                        @if ($traceInfo?->status == 1)
                            ...
                            <a href="{{ route('warranty') }}?qr={{ $traceInfo?->order_no ?? '' }}&order_no={{ $traceInfo?->order_no ?? '' }}"
                        ...
                        <button type="button"
                            ...
                            <i class="bi bi-camera me-1"></i> {{ __('messages.traceability_certification.scan_other_order') }}
                        </button>
                    </div>
                </div>
            </div>
    ...
    <div class="order-btn-link d-flex align-items-center justify-content-end gap-2 mt-5">
        <div class="d-flex align-items-center gap-2">
            ...
            <a href="{{ route('traceability') }}"
                class="fw-600">{{ __('messages.traceability_certification.check_other_order') }}</a>
        </div>
    </div>
</div>
<!-- Modal -->
            ...
            <div class="modal-content bg-img-cover"
                ...
                <div class="modal-body text-center text-white">
                    ...
                    <p class="fs-14 fw-500 mt-4 mb-2">
                        {{ __('messages.traceability_certification.customer_name_label') }}:
                        {{ $traceInfo?->customer_name ?? 'N/A' }}
                    </p>
                    <p class="fs-14 fw-500 mt-2 mb-2">{{ __('messages.traceability_certification.order_no_label') }}:
                        {{ $traceInfo?->order_no ?? 'N/A' }}
                    </p>
                    <p class="fs-14 fw-500 mt-2 mb-2">Phone: {{ $traceInfo?->phone ?? 'N/A' }}</p>
                    <p class="fs-14 fw-500 mt-2 mb-2">Email: {{ $traceInfo?->email ?? 'N/A' }}</p>
