@extends('langding.index')
@section('title', $category['name'] ?? __('messages.store_system'))
@section('langding_content')
<div class="page-branch">
    <div class="box-location pb-0">
        <div class="container-fluid">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('home') }}"
                            class="fs-15 text-black">{{ __('messages.home') }}</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('demo-branch') }}"
                            class="fs-15 text-black">{{ __('messages.news') }}</a></li>
                    <li class="breadcrumb-item active fs-15 text-black" aria-current="page">
                        {{ __('messages.store_system') }}
                    </li>
                </ol>
            </nav>
            <h2 class="font-hanzel fs-32 fw-400 text-center mt-4 title">{{ __('messages.store_system') }}
            </h2>
            <div class="box-location-intro text-center fs-14 mt-4 mx-auto">
                {{ __('messages.distribution_intro') }}<br>
                {{ __('messages.distribution_intro_2') }}
            </div>
            @include('langding.components.dealer-search-card', ['kicker' => __('messages.store_system')])
        </div>
    </div>

    <div class="branch-content-item">
        <div class="container-fluid">
            <div class="row map-fullscreen" id="dealerLayoutRow">
                <div class="col-xl-6 dealer-list-column d-none" id="dealerListColumn">
                    <div class="branch-content-location">
                        <div class="branch-content-location-title d-flex align-items-center justify-content-between border-bottom border-dark-subtle">
                            <div class="fs-28 fw-700 text-uppercase title">
                                {{ __('messages.stores_dealers') }}
                                <span class="dealer-count">(<span id="dealer-count">0</span>)</span>
                            </div>
                        </div>
                        <ul class="nav nav-pills d-block" role="tablist" aria-orientation="vertical" id="dealerListShow"></ul>
                        <div id="noResults" class="text-center py-5" style="display: none;">
                            <h4 class="text-muted fs-18">{{ __('messages.no_dealers_found') }}</h4>
                            <p class="text-muted fs-14">{{ __('messages.try_another_keyword') }}</p>
                        </div>
                    </div>
                </div>

                <div class="col-12 dealer-map-column position-relative" id="dealerMapColumn">
                    <div id="mapbox"></div>
                    <div class="left-map position-absolute">Casumina.com.vn</div>
                    <div class="right-map position-absolute">Casumina.com.vn</div>
                </div>
            </div>
        </div>
    </div>
</div>

@include('langding.components.category-search-links')

@include('langding.components.promotion-slide')


@endsection

@include('langding.components.mapbox-assets')

@push('styles')
<style>
    .page-branch .branch-content-location .nav-pills .nav-link-item .map-link {
        height: 54px;
        width: 54px;
        border: 1px solid #6D6D6D;
        color: var(--bs-black);
        background-color: var(--bs-white);
        text-align: center;
        border-radius: 50%;
        display: flex;
        align-items: center;
        justify-content: center;
        padding: 0;
    }

    .page-branch .branch-content-location .nav-pills .nav-link-item-info {
        flex: 1 1 auto;
        min-width: 0;
        padding-right: 12px;
    }

    .page-branch .branch-content-location .btn-choose {
        flex-shrink: 0;
    }

    .page-branch .branch-content-location .nav-pills .nav-link-item .map-link:hover {
        background-color: #f0f0f0;
        border-color: #6D6D6D;
        color: var(--bs-black);
    }

    .page-branch .branch-content-location .nav-pills {
        max-height: 737px;
        overflow-y: auto;
    }

    .page-branch #mapbox {
        height: 800px;
        width: 100%;
    }

    .page-branch .map-fullscreen #mapbox {
        height: calc(100vh - 120px);
        min-height: 700px;
    }

    @media (max-width: 1199.98px) {
        .page-branch .branch-content-location .nav-pills {
            max-height: 600px;
        }

        .page-branch .branch-content-location .btn-choose {
            gap: .75rem !important;
            align-items: center;
        }

        .page-branch .branch-content-location .nav-pills .nav-link-item .map-link,
        .page-branch .branch-content-location .nav-pills .hot-phone {
            width: 38px;
            height: 38px;
            flex-shrink: 0;
        }

        .page-branch .branch-content-location .nav-pills .nav-link-item .map-link svg,
        .page-branch .branch-content-location .nav-pills .hot-phone svg {
            width: 18px;
            height: 18px;
        }

        .page-branch #mapbox {
            height: 600px;
        }

        .page-branch .map-fullscreen #mapbox {
            height: 600px;
            min-height: 600px;
        }
    }

    @media (max-width: 575.98px) {
        .page-branch .branch-content-location .nav-pills {
            max-height: 400px;
        }

        .page-branch #mapbox {
            height: 400px;
        }

        .page-branch .map-fullscreen #mapbox {
            height: 300px;
            min-height: 300px;
        }
    }
</style>
@endpush

@push('scripts')
<script>
    window.branchPageConfig = {
        dealers: @json($dealers ?? []),
        routes: {
            searchDealers: @json(route('api.search-dealers')),
            searchNearestDealers: @json(route('api.search-nearest-dealers')),
            searchAllDealers: @json(route('api.search-all-dealers')),
        },
        locale: @json(app()->getLocale()),
        countries: @json($countries ?? []),
        csrfToken: @json(csrf_token()),
        translations: {
            pleaseSelectProvince: @json(__('messages.please_select_province')),
            searching: @json(__('messages.searching')),
            directions: @json(__('messages.directions')),
            contact: @json(__('messages.contact')),
            products: @json(__('messages.products')),
            provinceCity: @json(__('messages.province_city')),
            nearestYou: @json(__('messages.nearest_you')),
            casuminaDealer: @json(__('messages.casumina_dealer')),
            noDealersFound: @json(__('messages.no_dealers_found')),
            tryAnotherKeyword: @json(__('messages.try_another_keyword')),
            locationNotSupported: @json(__('messages.location_not_supported')),
            locationError: @json(__('messages.location_error')),
            pleaseSelectLocationCategory: @json(__('messages.please_select_location_category')),
        },
    };
</script>
<script src="{{ asset('langding/js/mapbox.js') }}"></script>

@endpush